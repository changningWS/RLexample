# Actor Critic for openai-gym Pong game
import os
import argparse
import gym
import numpy as np
...
parser = argparse.ArgumentParser(description='Actor Critic demo')
...
def finish_episode():
    ...
    for (log_prob, value), reward in zip(flatten_log_probs, rewards):
        ...
        value_loss.append(F.smooth_l1_loss(value.reshape(-1), reward.reshape(-1))) # 值函数近似
    optimizer.zero_grad()
