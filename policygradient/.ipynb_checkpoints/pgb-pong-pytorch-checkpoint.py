# policy gradient with baseline for openai-gym Pong game
...
parser = argparse.ArgumentParser(description='Policy gradient  with baseline demo')
...
def finish_episode():
    ...
    for (log_prob, value), reward in zip(policy.saved_log_probs, rewards):
        advantage = reward - value #value作为baseline
        #同时去优化策略函数和价值函数
        policy_loss.append(- log_prob * advantage)         # 策略梯度
        value_loss.append(F.smooth_l1_loss(value, reward)) # 值函数近似
    optimizer.zero_grad()
...
for i_episode in count(1):
    ...
    for t in range(10000):
        signal.signal(signal.SIGINT, CtrlCHandler) #目的是让程序中断时，gym窗口也随之关闭
